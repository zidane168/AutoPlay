...
TargetList.OnFrameKeyDown = function()
...
-- on mouse hover list
TargetList.OnItemMouseEnter = function()
	local imgOver = this:Lookup("Image_LOver")
	imgOver:SetSize(240,25)
	imgOver:Show()		
end

-- on mouse leave
TargetList.OnItemMouseLeave = function()
	local imgOver = this:Lookup("Image_LOver")
	imgOver:Hide()
end

-- on item Lbuttondown
TargetList.OnItemLButtonDown = function()
	local szName = this:GetName()
	-- OutputMessage ("MSG_SYS", "szName = " .. szName)
	-- if string.sub(szName,1,4)=="List" then
		...
		SetTarget(dwType,this.dwID);
	-- end
end

TargetList.OnItemRButtonDown = function()	
	-- VAutoRepair.OpenDlg();
end
