...
VAutoRepair.OnFrameKeyDown = function()
...
VAutoRepair.OpenDlg = function()
	...
	local _frame = Station.Lookup("Normal/VAutoRepair") or Wnd.OpenWindow(_szIniFile, "VAutoRepair")
	_frame:Show();	
	_frame:SetPoint("CENTER", 0, 0, "CENTER", 0, 0);
	_frame:BringToTop();
	
	local _container = Station.Lookup("Normal/VAutoRepair/Window_Main");
...
VAutoRepair.OnSell = function(nShopID, nShopType, nValidPageCount, nCanRepair, nNpcID)
	...
	if (VAutoRepair.bIsGrey == false and VAutoRepair.bIsWhite == false and VAutoRepair.bIsGreen == false and VAutoRepair.bIsBlue == false and VAutoRepair.bIsBook == false and VAutoRepair.bIsMaterial == false ) 		then 
		return 
	end
			...
			if item and item.bCanTrade then				
				...
				if VAutoRepair.bIsBook then
					if (item.nGenre == ITEM_GENRE.BOOK) then										
						if player.IsBookMemorized(GlobelRecipeID2BookID(item.nBookID)) then						
							bSell = true;						
						end
					end
				end				
